...
market_data.columns = ["DOLAR", "BTC", "EURO", "IBOVESPA", "sp500", "NASDAQ"]
...
def salvar_grafico(nome, dados):
    ...
    plt.plot(dados)
    plt.title(nome)
    file_path = os.path.join(output_path, f"{nome.lower()}.png")
...
salvar_grafico("sp500", market_data["sp500"])
...
sp500_return = daily_returns["sp500"].iloc[-1]
...
def send_email_from_gmail():
    ...
    <body>
        ...
        <ul>
            ...
            <li>O S&P 500 teve o retorno de {sp500_return}, fechando em {market_data["sp500"].iloc[-1].round(2)}.</li>
    ...
    for nome in ["ibovespa", "dolar", "sp500", "btc", "euro", "nasdaq"]:
        file_path = os.path.join(output_path, f"{nome}.png")
        if not os.path.exists(file_path):
            print(f"File not found: {file_path}")
            continue  # Skip attaching this image if it doesn't exist
...
#     - S&P 500: {sp500_return} - Fechou em {market_data["sp500"].iloc[-1].round(2)}
